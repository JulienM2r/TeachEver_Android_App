...
public interface EleveDAO {
     ...
     */
    @Query("SELECT * FROM " + Eleve.TABLE_NAME + " WHERE " + Eleve.COLUMN_Email + " = :email AND " + Eleve.COLUMN_Password + " = :password LIMIT 1" )
    ...
    /**
     * Check if an email exist allready.
     *
     * @param email The column email.
     * @return A number of the selected email.
     */
    @Query("SELECT COUNT(*) FROM " + Eleve.TABLE_NAME + " WHERE " + Eleve.COLUMN_Email + " = :email")
    int selectEleveByEmail(String email);
    ...
    /**
     * Delete all eleve from eleves     *
     */
    @Query("DELETE FROM " + Eleve.TABLE_NAME)
    void deleteAll();
